...
export async function GET(request: NextRequest) {
  ...
  const { data: project } = await admin.from('projects').select('id,settings').eq('api_key_hash', await hashProjectApiKey(projectKey!)).maybeSingle()
  if (!project) {
  ...
  const { data: updateSettings } = await admin.from('product_update_settings').select('*').eq('project_id', project.id).maybeSingle()
  ...
  const runtimeVersion = request.nextUrl.searchParams.get('runtimeVersion')?.slice(0, 64) || null
  ...
  if (!installation || Date.now() - new Date(installation.last_seen_at).getTime() > 1000 * 60 * 15) {
    void admin.from('project_embed_installations').upsert({ project_id: project.id, last_seen_at: new Date().toISOString(), runtime_version: runtimeVersion, feedback_enabled: feedback, updates_enabled: updates }, { onConflict: 'project_id' })
  }
