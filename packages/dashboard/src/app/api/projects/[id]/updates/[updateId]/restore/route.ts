...
export async function POST(_: NextRequest, { params }: { params: Promise<{ id: string; updateId: string }> }) {
  ...
  const { data, error } = await auth.admin.from('product_updates').update({ status: 'draft', published_at: null, expires_at: null }).eq('project_id', id).eq('id', updateId).select('*').maybeSingle()
