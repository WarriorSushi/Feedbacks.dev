...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string; updateId: string }> }) {
  ...
  const parsed = sanitizeProductUpdateInput(body); if (Object.keys(parsed.errors).length) return NextResponse.json({ errors: parsed.errors }, { status: 400, headers })
...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string; updateId: string }> }) {
  ...
  if (data.image_path) await auth.admin.storage.from('product_update_images').remove([data.image_path])
