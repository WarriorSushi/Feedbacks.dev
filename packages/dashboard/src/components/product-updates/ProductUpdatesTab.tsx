...
export function ProductUpdatesTab({ projectId, projectKey, view = 'overview', updateId }: { projectId: string; projectKey: string | null; view?: 'overview' | 'composer' | 'settings'; updateId?: string }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Unable to load release notes.')
      ...
      toast({ title: 'Could not load release notes', description: message, variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }, [projectId])
  ...
  if (loading) return <Card><CardContent className="p-6 text-sm text-muted-foreground">Loading release notes…</CardContent></Card>
  if (loadError) return <Card className="mx-auto max-w-2xl"><CardContent className="p-6"><h2 className="text-xl font-semibold">Release notes could not load</h2><p className="mt-2 text-sm text-muted-foreground">{loadError}</p><Button className="mt-5" onClick={() => void load()}>Try again</Button></CardContent></Card>
  ...
  if (updateId && !updates.some((update) => update.id === updateId)) {
    return <Card className="mx-auto max-w-2xl"><CardContent className="p-6"><h2 className="text-xl font-semibold">Release note not found</h2><p className="mt-2 text-sm text-muted-foreground">It may have been deleted, or the link may be out of date.</p><Button className="mt-5" variant="outline" onClick={() => router.push(`/projects/${projectId}/release-notes`)}>Return to release notes</Button></CardContent></Card>
  }

  if (publishConfirmation) {
    const scheduled = publishConfirmation === 'scheduled'
    return <><Card className="mx-auto max-w-2xl"><CardContent className="p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Release notes</p><h2 className="mt-2 text-2xl font-semibold">{scheduled ? 'Scheduled successfully' : 'Published successfully'}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{scheduled ? `This release note becomes eligible at ${new Date(publishAt).toLocaleString()}.` : 'This release note is eligible now and will appear on matching pages for visitors who have not seen it.'}</p><div className="mt-6 flex flex-wrap gap-2"><Button onClick={openPrivateTest}>Open private test</Button><Button variant="outline" onClick={() => router.push(`/projects/${projectId}/release-notes`)}>Return to release notes</Button></div></CardContent></Card>{privateTestOpen && <PrivateTestDialog form={form} settings={settings} onClose={() => setPrivateTestOpen(false)} />}</>
  }
  ...
  return <div className="space-y-6">
    <section className="flex flex-wrap items-start justify-between gap-4 border-b pb-5">
      <div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Release notes</p><h2 className="mt-1 text-xl font-semibold">{selected ? 'Edit release note' : 'Create release note'}</h2><p className="mt-1 text-sm text-muted-foreground">Keep the announcement concise. You can review it privately before publishing.</p></div>
      <Button variant="outline" onClick={() => router.push(`/projects/${projectId}/release-notes`)}>Back to release notes</Button>
    </section>
