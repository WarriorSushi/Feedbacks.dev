...
export function ProductUpdatesTab({ projectId, projectKey, view = 'overview', updateId }: { projectId: string; projectKey: string | null; view?: 'overview' | 'composer' | 'settings'; updateId?: string }) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      toast({ title: 'Could not load Product Updates', description: message, variant: 'destructive' })
    } finally {
      setLoading(false)
    }
  }, [projectId])
  ...
  if (loadError) return <Card className="mx-auto max-w-2xl"><CardContent className="p-6"><h2 className="text-xl font-semibold">Updates could not load</h2><p className="mt-2 text-sm text-muted-foreground">{loadError}</p><Button className="mt-5" onClick={() => void load()}>Try again</Button></CardContent></Card>
  ...
  if (view === 'settings') {
    return <UpdatesSettings settings={settings} entitlements={entitlements} onSave={saveSettings} onBack={() => router.push(`/projects/${projectId}/updates`)} />
  }
  if (view === 'overview') {
    return <UpdatesOverview
      updates={updates}
      onNew={() => router.push(`/projects/${projectId}/updates/new`)}
      onEdit={(id) => router.push(`/projects/${projectId}/updates/${id}`)}
      onSettings={() => router.push(`/projects/${projectId}/updates/settings`)}
  ...
  if (updateId && !updates.some((update) => update.id === updateId)) {
    return <Card className="mx-auto max-w-2xl"><CardContent className="p-6"><h2 className="text-xl font-semibold">Update not found</h2><p className="mt-2 text-sm text-muted-foreground">It may have been deleted, or the link may be out of date.</p><Button className="mt-5" variant="outline" onClick={() => router.push(`/projects/${projectId}/updates`)}>Return to Updates</Button></CardContent></Card>
  }

  if (publishConfirmation) {
    const scheduled = publishConfirmation === 'scheduled'
    return <><Card className="mx-auto max-w-2xl"><CardContent className="p-6"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">Updates</p><h2 className="mt-2 text-2xl font-semibold">{scheduled ? 'Scheduled successfully' : 'Published successfully'}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{scheduled ? `This update becomes eligible at ${new Date(publishAt).toLocaleString()}.` : 'This update is eligible now and will appear on matching pages for visitors who have not seen it.'}</p><div className="mt-6 flex flex-wrap gap-2"><Button onClick={openPrivateTest}>Open private test</Button><Button variant="outline" onClick={() => router.push(`/projects/${projectId}/updates`)}>Return to Updates</Button></div></CardContent></Card>{privateTestOpen && <PrivateTestDialog form={form} settings={settings} onClose={() => setPrivateTestOpen(false)} />}</>
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      ...
      <aside className="space-y-6 xl:sticky xl:top-6 xl:self-start"><section className="border rounded-lg p-4"><div className="flex items-center justify-between"><h3 className="font-semibold">Preview</h3><div className="flex gap-1"><Button size="sm" variant={previewMobile ? 'outline' : 'secondary'} onClick={() => setPreviewMobile(false)}>Desktop</Button><Button size="sm" variant={previewMobile ? 'secondary' : 'outline'} onClick={() => setPreviewMobile(true)}>Mobile</Button></div></div><div className="mt-3 flex items-center justify-between"><span className="text-xs text-muted-foreground">{settings?.theme === 'auto' ? 'Auto theme' : `${settings?.theme || 'Auto'} theme`}</span><Button size="sm" variant="ghost" onClick={() => setPreviewDark((value) => !value)}><Eye className="mr-1 h-3.5 w-3.5" />{previewDark ? 'Light' : 'Dark'}</Button></div><UpdatePreview form={form} dark={previewDark} mobile={previewMobile} accent={settings?.accentColor || '#6366f1'} /><Button className="mt-3 w-full" variant="outline" onClick={openPrivateTest}>Open private test</Button></section>
        ...
        <Button className="w-full" variant="ghost" onClick={() => router.push(`/projects/${projectId}/updates/settings`)}><Settings2 className="mr-2 h-4 w-4" />Updates settings</Button>
      </aside>
    </div>
