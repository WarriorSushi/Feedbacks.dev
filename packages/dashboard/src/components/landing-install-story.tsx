...
const steps = [
  { title: 'Configure', detail: 'Shape the form in feedbacks.dev' },
  { title: 'Install', detail: 'Paste one stable embed' },
  { title: 'Verify', detail: 'Send one private test' },
] as const
    ...
    <div className="overflow-hidden border-y border-zinc-800 bg-zinc-950 text-zinc-100 shadow-[var(--shadow-float)] sm:rounded-[16px] sm:border">
      <div className="grid border-b border-white/10 sm:grid-cols-3">
        {steps.map((step, index) => (
          <button key={step.title} type="button" onClick={() => { setActive(index); setPaused(true) }} aria-pressed={active === index} className={cn('relative flex min-h-[72px] items-center gap-3 border-white/10 px-5 text-left transition-colors sm:border-l sm:first:border-l-0', index > 0 && 'border-t sm:border-t-0', active === index ? 'bg-white/[0.07]' : 'hover:bg-white/[0.03]')}>
        ...
        <div key={active} className="animate-fade-in">
          {active === 0 && (
            <div className="grid min-h-[282px] items-center gap-6 md:grid-cols-[0.8fr_1.2fr]">
              <div className="px-2 sm:px-4">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-lime-300">Remote configuration</p>
                <h3 className="mt-3 text-xl font-semibold">Make it feel native before it goes live.</h3>
                <p className="mt-3 max-w-sm text-xs leading-5 text-zinc-400">Choose the form fields, words, accent, and launcher once. Future edits publish from the dashboard without touching your code.</p>
              </div>
              <div className="border border-white/10 bg-[#18191c]">
                ...
                <div className="divide-y divide-white/[0.07] px-4">
                  ...
                  <div className="flex items-center justify-between py-4"><span className="inline-flex items-center gap-2 text-xs text-zinc-300"><Radio className="h-3.5 w-3.5 text-zinc-400" /> Product updates</span><span className="rounded-full bg-lime-300/10 px-2 py-1 text-[10px] text-lime-300">Ready when you are</span></div>
                </div>
              </div>
            </div>
          )}
          ...
          {active === 1 && (
            <div className="grid min-h-[282px] items-center gap-6 md:grid-cols-[0.72fr_1.28fr]">
              <div className="px-2 sm:px-4">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-lime-300">The only code step</p>
                <h3 className="mt-3 text-xl font-semibold">Copy once. Keep control remotely.</h3>
                <p className="mt-3 max-w-sm text-xs leading-5 text-zinc-400">The Website snippet connects this product to its saved configuration. The Copy button stays visible and the public project key is safe for browser code.</p>
              </div>
              <div className="min-w-0">
                <div className="mb-3 flex items-center justify-between text-[10px] text-zinc-400"><span className="inline-flex items-center gap-2"><Code2 className="h-3.5 w-3.5" /> Website snippet</span><span>Install once</span></div>
          ...
          {active === 2 && (
            <div className="grid min-h-[282px] items-center gap-6 md:grid-cols-[0.8fr_1.2fr]">
              <div className="px-2 sm:px-4">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-lime-300">Private verification</p>
                <h3 className="mt-3 text-xl font-semibold">Know it works before users see it.</h3>
                <p className="mt-3 max-w-sm text-xs leading-5 text-zinc-400">Open your product, send one test message, and confirm the exact page and browser context arrived. Then publish with confidence.</p>
              </div>
              <div className="border border-lime-300/20 bg-lime-300/[0.055] p-5 sm:p-6">
                ...
                <div className="mt-6 grid gap-3 border-t border-white/10 pt-5 sm:grid-cols-2"><span className="inline-flex items-center gap-2 text-[10px] text-zinc-400"><ShieldCheck className="h-3.5 w-3.5 text-lime-300" /> Browser-safe install key</span><span className="inline-flex items-center gap-2 text-[10px] text-zinc-400"><Radio className="h-3.5 w-3.5 text-lime-300" /> Remote changes connected</span></div>
              </div>
            </div>
          )}
        </div>
